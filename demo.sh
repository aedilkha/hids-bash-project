...
demo_success "Baseline created"

# Step 2: Verify clean state
demo_section "Step 2: Running baseline check (expect no alerts)"
printf 'Full scan of healthy system...\n'
full_output=$("$HIDS_SCRIPT" --config "$SCRIPT_DIR/hids.demo.conf" --no-color 2>&1)
if echo "$full_output" | grep -q "No anomaly detected"; then
    demo_success "Clean system confirmed - no alerts"
else
    demo_error "Unexpected alerts detected"
    echo "$full_output" | tail -10
fi

# Step 3: Scenario A - Process from /tmp
demo_section "Scenario A: Malicious Process from /tmp"
printf 'Launching temporary executable...\n'
"$TOOLS_DIR/simulate_attack.sh" 2>&1 | grep -E "^(Simulated|Running|Demo)" || true

sleep 2
printf '\nScanning with Module 3...\n'
module3_output=$("$HIDS_SCRIPT" --config "$SCRIPT_DIR/hids.demo.conf" --module 3 --no-color 2>&1)
...
printf 'Modifying sensitive file permissions...\n'
if bash "$TOOLS_DIR/demo_file_tampering.sh" 2>&1 | head -3; then
    sleep 2
    printf '\nScanning with Module 4...\n'
    module4_output=$("$HIDS_SCRIPT" --config "$SCRIPT_DIR/hids.demo.conf" --module 4 --no-color 2>&1)
...
if [[ -f /var/log/hids/alerts.log ]]; then
    tail -10 /var/log/hids/alerts.log | while read -r line; do
        printf '%s%s%s\n' "$C_YELLOW" "$line" "$C_RESET"
    done
else
    printf 'No alerts log found at /var/log/hids/alerts.log\n'
fi
