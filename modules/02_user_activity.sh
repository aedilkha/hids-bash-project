...
user_account_snapshot() {
    ...
    awk -F: '{print "ACCOUNT|" $1 "|" $3 "|" $4 "|" $6 "|" $7}' "${PASSWD_FILE:-/etc/passwd}" | sort
}
...
user_check_uid_gid_duplicates() {
    ...
    while read -r id names; do
        ...
        ((USER_FINDING_TOTAL++))
    done < <(awk -F: '{names[$3]=names[$3] $1 " "; count[$3]++} END {for (id in count) if (count[id] > 1) print id, names[id]}' "${PASSWD_FILE:-/etc/passwd}")
    while read -r id names; do
        [[ -z "${id:-}" ]] && continue
...
user_shadow_snapshot() {
    ...
    awk -F: '{state=($2==""?"EMPTY":($2~/^[!*]/?"LOCKED":"ACTIVE")); print "SHADOW|" $1 "|" state "|" $3 "|" $5 "|" $8}' "${SHADOW_FILE:-/etc/shadow}" | sort
}
...
user_check_shadow() {
    ...
    if [[ ! -r "${SHADOW_FILE:-/etc/shadow}" ]]; then
...
user_group_snapshot() {
    ...
    for group in sudo wheel; do
        ...
        gid="$(cut -d: -f3 <<< "$entry")"
        members="$( { cut -d: -f4 <<< "$entry" | tr ',' '\n'; awk -F: -v gid="$gid" '$4 == gid {print $1}' "${PASSWD_FILE:-/etc/passwd}"; } | sed '/^$/d' | sort -u | tr '\n' ',')"
        printf 'GROUP|%s|%s|%s\n' "$group" "$gid" "$members"
    done
}
